void loadingSetting(){
    ...
    for(int i=0; i<lines.length; i++){
        if(!lines[i].equals("") && !lines[i].startsWith("#")){
            ...
            if(lines[i].startsWith("videoFileName=") && lines[i].endsWith(";")){
                String value = lines[i].substring("videoFileName=".length(), lines[i].indexOf(";"));
...
//此範例不需要使用mpeHackForCascadePlayer()
